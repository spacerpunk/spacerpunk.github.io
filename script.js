...
function updateInitAudioButton() {
  ...
  initAudioButton.title = audioIsInit
    ? 'Audio Initialized'
    : 'Initialize Audio';
}
...
async function processCommand(cmd) {
  ...
  if (terminalCommands.hasOwnProperty(lowerCmd)) {
    if (lowerCmd === 'visual') {
    ...
    } else if (lowerCmd === 'portfolio' || 'page' || 'newpage') {
      addOutput(terminalCommands[lowerCmd]);
      setTimeout(() => {
        window.location.href =
          'https://spacerpunk.github.io/spacerpunk-portfolio/'; // Replace with your desired HTML file
      }, 600); // Delay for 1.5 seconds to show the message
      ...
      if (
        window.MusicPieceOne &&
        audioIsInit &&
        typeof window.MusicPieceOne.start === 'function'
      ) {
        try {
          ...
          addOutput(
            '<span style="color:#ff0000">Error: Could not start music. Please try again or initialize audio first.</span>'
          );
        }
      } else {
        console.error('MusicGenerator.start is not a function');
        addOutput(
          '<span style="color:#ff0000">Error: Music system not initialized properly, press the ▶ on the right :)</span>'
        );
      }
      ...
      if (
        ...
        typeof window.MusicGenerator.stop === 'function'
      ) {
      ...
      addOutput(terminalCommands[lowerCmd]);
      logMessage(
        '<iframe style="border-radius:1px" src="https://open.spotify.com/embed/artist/6pUpuBZFid3AGpgTOnrIYr?utm_source=generator&theme=0" width="700px" height="100px" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>'
      );
    ...
    addOutput(
      `<span class="error" style="color:#ff0000">Command not recognized:&nbsp; </span>${cmd}`
    );
  }
...
function typeText(html, speed = 15, callback = null, className = '') {
  ...
  function type() {
    if (currentIndex < textContent.length) {
      ...
      let currentHTML = html.substring(
        0,
        html.indexOf(currentChar) + currentIndex + 1
      );
...
function updateTheme() {
  ...
  document
    .querySelectorAll('.output-line, .input-line, .prompt')
    .forEach((el) => {
