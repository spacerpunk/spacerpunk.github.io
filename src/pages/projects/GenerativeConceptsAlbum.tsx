import SEO from '../../components/SEO';

export default function GenerativeConceptsAlbum() {
  return (
    <div className="w-full p-4 md:p-6">
      <SEO
        title="Generative Concepts Album"
        description="An experimental album created entirely through generative systems and AI collaboration"
      />
      <h1 className="text-3xl md:text-4xl font-bold mb-4">
        Generative Concepts Album
      </h1>
      <div>
        <h3 className="font-extralight text-sm py-1 text-nasared">
          #generativemusic #ai #album #experimental
        </h3>

        <div className="py-6">
          <h2 className="text-xl font-semibold mb-4">About</h2>
          <p className="text-gray-400 leading-relaxed">
            A full-length album exploring generative music techniques and
            AI-assisted composition. Each track is a collaboration between human
            creativity and algorithmic processes, pushing the boundaries of
            music creation.
          </p>
        </div>

        <div className="py-4">
          <p className="text-sm text-gray-500">Coming soon...</p>
        </div>
      </div>
    </div>
  );
}
